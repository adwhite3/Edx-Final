---
title: "Australian Rain Predictor"
author: "Andrew White"
date: "1/10/2020"
output:
...
data<-read_csv(url)
```
I will be making a classification tree to predict if there will or will not be precipitation tomorrow in Austrailia
...
Next I will check if Pressure at 3pm is a good predictor
```{r}
...
```
As the violin chart shows, below 1012hpa, it is more likely to rain tomorrow
...
```
Using a confusionMatrix, I can check the metrics for success.
The decision tree model has an 83.83% accuracy which is pretty good. However, the Sensitivity is 42.7% which is a bit low and the Balanced Accuracy is only 69.05%. But by using Random Forrests I can improve the balanced accuracy and sensitivity
